...
-- Step 3: Get opening and closing comments using LATERAL JOINs for better performance
...
SELECT
  ...
  COALESCE(dwh.clean_comment_for_csv(cd.opening_body), '') AS opening_comment,
...
-- Cleanup: Drop the helper function if it was created just for this query
-- (Comment out if you want to keep it for reuse)
-- DROP FUNCTION IF EXISTS dwh.clean_comment_for_csv(TEXT);
