...
**Note**: The OSM Notes ecosystem consists of 8 projects working together to provide a complete OSM
Notes analysis solution. OSM-Notes-Ingestion is the base project, as it was the first created and
provides the foundation for all others.
...
**Note**: The JSON exports are consumed by OSM-Notes-Viewer (part of the 8-project ecosystem), which
provides the web interface for visualizing these analytics. Viewer consumes data from OSM-Notes-Data
(GitHub Pages), which is populated by Analytics export scripts.
...
- **Relationship**: Viewer consumes JSON files from OSM-Notes-Data (GitHub Pages), which is
  populated by Analytics export scripts
...
- [OSM-Notes-Viewer](https://github.com/OSM-Notes/OSM-Notes-Viewer) - Ecosystem project (downstream,
  consumes Data)
- [OSM-Notes-Data](https://github.com/OSM-Notes/OSM-Notes-Data) - JSON data files (populated by
  Analytics)
