...
This trigger calculates accumulated historical metrics for each note action BEFORE inserting a new row into `dwh.facts`. It populates three columns:
...
These metrics enable **HISTORICAL SNAPSHOT ANALYSIS** - understanding what the state of a note was at ANY specific point in time, not just the current state.
