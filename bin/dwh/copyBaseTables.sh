...
INGESTION_USER="${DB_USER_INGESTION:-${DB_USER:-postgres}}"
...
if ! psql -d "${INGESTION_DB}" -U "${INGESTION_USER}" -c "SELECT 1;" > /dev/null 2>&1; then
...
psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -c "CREATE SCHEMA IF NOT EXISTS public;" > /dev/null 2>&1 || true
...
for table in "${TABLES[@]}"; do
 ...
 if ! psql -d "${INGESTION_DB}" -U "${INGESTION_USER}" -t -c \
 ...
 if psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -t -c \
  ...
  psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -c "DROP TABLE IF EXISTS public.${table} CASCADE;" > /dev/null 2>&1 || true
 fi

 # Get table structure and create in target
 __logi "Creating table structure for ${table}"
 pg_dump -d "${INGESTION_DB}" -U "${INGESTION_USER}" \
  -t "public.${table}" \
  --schema-only \
  --no-owner --no-acl \
  | psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -q > /dev/null 2>&1 || {
 ...
 row_count=$(psql -d "${INGESTION_DB}" -U "${INGESTION_USER}" -t -c "SELECT COUNT(*) FROM public.${table};" | tr -d ' ')
 __logi "Copying ${row_count} rows from ${table}"

 if ! psql -d "${INGESTION_DB}" -U "${INGESTION_USER}" \
  -c "\COPY public.${table} TO STDOUT" 2> /dev/null \
  | psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" \
 ...
 # Verify row count
 target_count=$(psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -t -c "SELECT COUNT(*) FROM public.${table};" | tr -d ' ')
 ...
 psql -d "${INGESTION_DB}" -U "${INGESTION_USER}" -t -A -c "
 ...
 " 2> /dev/null | while read -r index_sql; do
  if [[ -n "${index_sql}" ]]; then
   psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -c "${index_sql}" > /dev/null 2>&1 || {
 ...
 psql -d "${ANALYTICS_DB}" -U "${ANALYTICS_USER}" -c "ANALYZE public.${table};" > /dev/null 2>&1 || true
